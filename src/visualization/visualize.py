...
# Comparing the medium vs the heavy sets
...
#Create a loop to plot all combinations per sensors
...
for label in labels:
    for participants in participants:
        all_axis_df = (
            ...
            .query(f"participant == '{participants}'")
            .reset_index()
        )
...
for label in labels:
    for participants in participants:
        ...
        if len(combined_plot_df) > 0:
            ...
            plt.savefig(f"../../reports/figures/{label.title()} ({participants}).png", bbox_inches="tight")
            plt.show()
